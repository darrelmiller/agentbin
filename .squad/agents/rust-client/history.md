...
- **(2026-04-11) Updated SDK from v1.0.18 → v1.0.19** for client against .NET server. Updated `tests/ClientTests/rust/Cargo.toml` — `a2a-rs-client` and `a2a-rs-core` bumped to v1.0.19. Build: `cargo build` succeeded cleanly with no API breakages. Test Results: **54/60 (90%)** against .NET server. Breakdown: JSON-RPC 25/27 (2 known failures: returnImmediately not supported in SDK, server feature gap), REST 25/27 (2 known: SDK limitations), v0.3 4/4. SDK v1.0.19 is production-ready and fully compatible with existing client code. Committed to main as 5e1c562.
