...
async fn main() {
    ...
    // ── REST binding tests (SDK 1.0.9+ supports REST) ────────────
...
async fn run_rest_tests(base_url: &str, results: &mut Vec<TestResult>) {
    ...
    // Build REST clients from discovered agent cards
    let echo_client = build_rest_client_for(base_url, "/echo").await;
    let spec_client = build_rest_client_for(base_url, "/spec").await;
    ...
    // ── 3. Echo Send Message ──
    {
        ...
        match &echo_client {
            Some(client) => {
                let msg = new_user_message("hello from Rust REST");
                match client.send_message(msg, None, None).await {
                    Ok(resp) => {
                        let text = extract_text(&resp);
                        let has_hello = text.to_lowercase().contains("hello");
                        record(
                            results,
                            "rest/echo-send-message",
                            "Echo Send Message",
                            has_hello,
                            &format!(
                                "response contains 'hello': {has_hello}, text={}",
            ...
            None => {
                record(
                    ...
                    "Echo Send Message",
                    false,
                    "echo REST client not available",
                    start.elapsed(),
                );
            }
        }
    ...
    // All remaining REST tests need the spec client
    let spec = match &spec_client {
        Some(c) => c,
        None => {
            let remaining = [
            ...
            for (id, name) in remaining {
                record(
                    ...
                    name,
                    false,
                    "spec REST client not available",
                    std::time::Duration::ZERO,
                );
            }
            return;
                ...
                if !task.status.state.is_terminal() {
                    match spec.cancel_task(&task.id, None).await {
                        Ok(cancelled) => {
                            ...
                            record(
                                results,
                                "rest/spec-cancel-with-metadata",
                                "Cancel With Metadata",
                                passed,
                                &format!("state={:?}", cancelled.status.state),
                                start.elapsed(),
    ...
    // ── 22. error-subscribe-not-found ──
    {
        ...
        match spec.subscribe_to_task(bogus_id, None).await {
            Ok(_) => {
                record(
                    results,
                    "rest/error-subscribe-not-found",
                    "Subscribe Not Found",
                    false,
                    "expected error, got stream",
                    start.elapsed(),
                ...
                if let Some(task_id) = tid1 {
                    ...
                    match spec.send_message(msg2, None, None).await {
                        Ok(resp2) => {
                            ...
                            record(
                                results,
                                "rest/multi-turn-context-preserved",
                                "Context Preserved",
                                preserved,
                                &format!(
                                    "contextId1={}, contextId2={}, match={}",
...
/// Build a REST transport client for the given agent path.
async fn build_rest_client_for(base_url: &str, path_prefix: &str) -> Option<A2aClient> {
    let server_url = format!("{base_url}{path_prefix}");

    // Manually fetch agent card to find the REST endpoint
    let card_url = format!("{server_url}/.well-known/agent-card.json");
    let http = reqwest::Client::new();
    let card_val: serde_json::Value = match http
        .get(&card_url)
        ...
        .await
    {
        Ok(resp) => match resp.error_for_status() {
            Ok(r) => match r.json().await {
                Ok(v) => v,
                Err(e) => {
                    eprintln!("  ⚠ REST: Failed to parse agent card at {card_url}: {e}");
                    return None;
                }
            },
            Err(e) => {
                eprintln!("  ⚠ REST: Failed to fetch agent card at {card_url}: {e}");
                return None;
            }
        },
        Err(e) => {
    ...
    // Find the REST endpoint from supportedInterfaces
    let endpoint = card_val
        ...
        .and_then(|ifaces| {
            ifaces.iter().find_map(|i| {
                ...
                if binding.eq_ignore_ascii_case("rest") || binding.eq_ignore_ascii_case("http+json") {
    ...
    let endpoint_url = match endpoint {
        ...
        None => {
            eprintln!(
                "  ⚠ Agent card at {card_url} has no REST endpoint in supportedInterfaces"
            );
            return None;
        }
    };
    ...
    match A2aClient::new(config) {
        ...
        Err(e) => {
            eprintln!("  ⚠ Failed to create REST client for {base_url}{path_prefix}: {e}");
            None
        }
    }
}
...
async fn build_client_for(base_url: &str, path_prefix: &str) -> Option<A2aClient> {
    ...
    // RPC client — SDK now sends A2A-Version: 1.0 automatically for V1_0
    let config = ClientConfig {
        server_url,
        max_polls: 30,
        poll_interval_ms: 2000,
...
/// Build a client for the v0.3 agent via SDK — use ProtocolVersion::V0_3
/// so the SDK applies the v0.3 compatibility layer automatically.
async fn build_v03_client(base_url: &str) -> Option<A2aClient> {
    ...
    let card_url = format!("{server_url}/.well-known/agent-card.json");
    let http = reqwest::Client::new();
    let card_val: serde_json::Value = match http.get(&card_url).send().await {
        Ok(resp) => match resp.error_for_status() {
            Ok(r) => match r.json().await {
                Ok(v) => v,
                Err(e) => {
                    eprintln!("  ⚠ v0.3: Failed to parse agent card at {card_url}: {e}");
                    return None;
                }
            },
            Err(e) => {
                eprintln!("  ⚠ v0.3: Failed to fetch agent card at {card_url}: {e}");
                return None;
            }
        },
        Err(e) => {
    ...
    // v0.3 cards use a top-level `url` field for the endpoint
    let endpoint = card_val
        .get("url")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    ...
    let endpoint_url = match endpoint {
        Some(url) => url,
        None => {
            eprintln!("  ⚠ v0.3 agent card at {card_url} has no 'url' field");
            return None;
        }
    };
